...
select a.BatchID,a.MatNo,a.volumeid,a.mattype,a.mattypename,a.mattypedesc,a.MatDesc,a.OrderNo,a.line,a.orderserial,a.lockflag,                                                                                                      
       ...
       --case when isnull(a.clothareano,'')<>'' then '是' else '否' end as isplated ,    
...
left join (select mm.docno,mm.batchid,matno,length,lunit,weight,wunit,conversion, turnoldragsflag,turnoldragsdate,                                                            
                  cc.CollectDate ,cc.color_name,cc.color_desc,mm.remark,'' as returndpt                                                                              
           from MaterialCollect_detail (nolock) mm inner join  MaterialCollect_head cc on mm.docno=cc.docno                                                                         
           union all                                                                                                 
           select rr.docno,newbatchid,rr.matno,length,lunit,weight,wunit,conversion  ,'','',                                                            
                  ss.InDate CollectDate,ss.ColorID  color_name,ss.ColorDesc color_desc ,rr.remark,returndpt                                                                                                 
           from MaterialReturnStore_Detail (nolock)  rr inner join  MaterialReturnStore_head ss on rr.docno=ss.docno                                                                                             
          ) d on a.batchid=d.BatchID                                         
