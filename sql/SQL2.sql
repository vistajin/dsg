       ...
       SELECT  a.sizegroup,a.styleno,a.styledesc,a.tracknm,a.goodsperiod,a.batchno,a.matioid,a.EstimateDate, b.basesizeuseqty,
       ...
       FROM ban_makebill_head a(nolock)   iNNER JOIN
            (select ISNULL(sc_number,'') as sc_number,max(isnull(mbversion,'')) as mbversion,mbtypeid,
                    ISNULL(xxxfield,'')  as xxxfield
              from ban_makebill_head (nolock)
             group by ISNULL(sc_number,'') ,mbtypeid,ISNULL(xxxfield,'')
            ) a1 on a.sc_number=a1.sc_number and isnull(a.mbversion,'') =a1.mbversion  and
                    a.mbtypeid=a1.mbtypeid  and isnull(a.xxxfield,'')=a1.xxxfield        inner join
            ban_makebill_itemlist b(nolock) on a.mb_number = b.mb_number       left join
            ban_makebill_itemcolor e (nolock) on  b.mb_number=e.mb_number and b.mb_line=e.mb_line  LEFT JOIN
            b_styleno c(nolock) on a.sc_number = c.sc_number left join
            b_item d(nolock) on b.item_code=d.item_code left join
            b_itemcate f (nolock) on d.treeid=f.treeid   left join
            s_users s(nolock) on a.trackgroupnmid=s.hruserid  left join
            MC_dsgdept m(nolock) on a.devdept=m.dsgdeptdesc LEFT JOIN
            ban_makebill_goods g on a.mb_number=g.mb_number and g.color_name=e.stylecolorid

            left join BanBatchAndCrockID(NOLOCK) h
            on h.colorID=e.itemcolorid and h.banid=a.matioid and h.MatNo=b.item_code
