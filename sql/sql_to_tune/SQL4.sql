SELECT distinct cast(c.isoldmat as bit) isoldmat,                          
 ...
 /*                            
 left join MaterialSend_head (nolock) resh on resh.docno=a.resno                             
 left join MaterialSend_DetailD (nolock) resd on resd.docno=a.resno  and resd.dline=a.resline                           
 */                          
                      left join(                          
 SELECT resh.docno,resh.Senddate,resh.senduser,resh.ReceiveDpt,                          
  resd.dline                          
 FROM MaterialSend_head (nolock) resh LEFT JOIN                          
   MaterialSend_DetailD (nolock) resd ON resh.docno=resd.docno                           
 ) res on res.docno=a.resno and res.dline=a.resline                            
                            
 left join (select c.batchid,sum(c.length) as length,sum(c.qtyi) as qtyi                                                                                 
  ...
  left join materialout_head (nolock) e on d.DocNo=e.SendNo                                                                                                              
   ...
   /*                          
   left join b_item (nolock) z on a.matno=z.item_code                              
   left join b_itemcate (nolock) x on  z.treeID=x.treeid                           
   */                            
   /*left join (                          
  SELECT a.item_code,a.itemtype,b.seriestype                           
    FROM b_item a LEFT JOIN                          
      b_itemcate b ON a.treeID=b.treeid                          
   WHERE usestate='启用'                             
   )item  on a.matno=item.item_code*/                              
   ...
   /* Modified By Mark 2016-7-22                                                                    
   left join materialreceivble_detail md (nolock) on a.batchid=md.BatchID                                                            
   left join materialreceivble_head hh (nolock) on md.DocNo=hh.DocNo                           
   Left join Rev_detail(nolock) rd on rd.rev_number=hh.rev_number and rd.req_number=hh.orderno and rd.req_line=hh.orderline                               
   */                          
   ...
   where 1=1 and a.DocNo not like 'RES%' 
