SELECT
    DISTINCT a.papermanager,a.PMPerson,a.newbanid
INTO #tmptb
...
SELECT
    DISTINCT b.papermanager,b.PMPerson,b.newbanid
INTO #tmpzjqs
...
FROM(                                             --头板
   ...
   )a INNER JOIN b_styleno b(nolock) on a.newbanid=b.banid WHERE 1=1


SELECT
    a.papermanager,a.PMPerson,a.newbanid,
...
FROM #tmpPaperQty a(nolock)
INNER JOIN dsg_bi.dbo.Ban_Lastver_MBT07 b(nolock) on a.newbanid=b.matioid
INNER JOIN ban_makebill_head c(nolock) on b.mb_number=c.mb_number
LEFT JOIN Paper_Banxing s01(nolock) on c.Stylenametype=s01.Stylenametype AND c.banxing=s01.banxing
LEFT JOIN Paper_Collartype s02(nolock) on c.Stylenametype=s02.Stylenametype AND c.Collartype=s02.Collartype
LEFT JOIN Paper_Design_style s03(nolock) on c.Stylenametype=s03.Stylenametype AND c.Design_style=s03.Design_style
LEFT JOIN Paper_Dresslength s04(nolock) on c.Stylenametype=s04.Stylenametype AND c.BX_length=s04.Dresslength
LEFT JOIN Paper_Fabcharacter s05(nolock) on c.Stylenametype=s05.Stylenametype AND c.Fabcharacter=s05.Fabcharacter
LEFT JOIN Paper_Fabname s06(nolock) on c.Stylenametype=s06.Stylenametype AND c.Fabname=s06.Fabname
LEFT JOIN Paper_SexType s07(nolock) on c.Stylenametype=s07.Stylenametype AND c.Sex=s07.SexType
LEFT JOIN Paper_SleeveLength s08(nolock) on c.Stylenametype=s08.Stylenametype AND c.outsidesleeve=s08.SleeveLength
LEFT JOIN Paper_Sleevetype s09(nolock) on c.Stylenametype=s09.Stylenametype AND c.Pants_type=s09.Sleevetype
LEFT JOIN Paper_Styletype2 s10(nolock) on c.Stylenametype=s10.Stylenametype AND c.Styletype2=s10.Styletype2
LEFT JOIN Paper_Thickness s11(nolock) on c.Stylenametype=s11.Stylenametype AND c.Thickness=s11.Thickness
LEFT JOIN Paper_Waist_type s12(nolock) on c.Stylenametype=s12.Stylenametype AND c.Waist_type=s12.Waist_type
WHERE 1=1


SELECT
    a.papermanager,a.PMPerson,a.newbanid as receiveBanid,
...
INTO #tmpQtyScore
FROM #tmpReceiveScore a
LEFT JOIN Paper_Perform_newBaseScore b ON a.newbanid=b.banid
WHERE 1=1
...
INTO #tmpPaperQat FROM (
    ...
    FROM (
        ...
        FROM(
            ...
            from PPPaperDept a(nolock)
            where a.bantype='落单' and a.qsdate>=dsg_bi.dbo.Calc_PaperTryFirstOK(a.newbanid) --签收时间须在试衣OK之后
            UNION
            select d.seniordesc,b.ComDateUser,c.newbanid,b.qsdate
            from PPPaperDept_detail b(nolock)
            INNER JOIN PPPaperDept c(nolock) ON b.PP_Number=c.PP_Number
            LEFT JOIN s_users d(nolock) ON b.ComDateUser=d.username
            where c.bantype='落单' and b.qsdate>=dsg_bi.dbo.Calc_PaperTryFirstOK(c.newbanid) --签收时间须在试衣OK之后
           )a
        GROUP BY  papermanager,PMPerson,newbanid
    ) a
    INNER JOIN b_styleno c(nolock) on a.newbanid=c.banid
    INNER JOIN ( --最早一次试衣OK资讯
    ...
    WHERE 1=1
        and a.qsdate>=d.scandatetime   --签收时间须在试衣OK之后
    ...
    FROM (
        ...
        FROM(
            select a.papermanager,a.PMPerson,a.newbanid,a.qsdate
            from PPPaperDept a(nolock)
            where a.bantype='制单' and a.qsdate>=dsg_bi.dbo.Calc_PaperTryFirstOK(a.newbanid) --签收时间须在试衣OK之后
            UNION
            select d.seniordesc,b.ComDateUser,c.newbanid,b.qsdate
            from PPPaperDept_detail b(nolock)
            INNER JOIN PPPaperDept c(nolock) ON b.PP_Number=c.PP_Number
            LEFT JOIN s_users d(nolock) ON b.ComDateUser=d.username
            where c.bantype='制单' and b.qsdate>=dsg_bi.dbo.Calc_PaperTryFirstOK(c.newbanid) --签收时间须在试衣OK之后
        )a
        GROUP BY  papermanager,PMPerson,newbanid
    ) a
    INNER JOIN b_styleno c(nolock) on a.newbanid=c.banid
...
SELECT
    ...
    ISNULL(s04.DresslengthScore,0)+ISNULL(s05.FabcharacterScore,0)+ISNULL(s06.FabnameScore,0)+
    ISNULL(s07.SexTypeScore,0)+ISNULL(s08.SleeveLengthScore,0)+ISNULL(s09.SleevetypeScore,0)+
    ISNULL(s10.Styletype2Score,0)+ISNULL(s11.ThicknessScore,0)+ISNULL(s12.Waist_typeScore,0) AS generalScore,
    ISNULL(s01.BanxingScore,0) BanxingScore,ISNULL(s02.CollartypeScore,0) CollartypeScore,
    ISNULL(s03.Design_styleScore,0) Design_styleScore,ISNULL(s04.DresslengthScore,0) DresslengthScore,
    ISNULL(s05.FabcharacterScore,0) FabcharacterScore,ISNULL(s06.FabnameScore,0) FabnameScore,
    ISNULL(s07.SexTypeScore,0) SexTypeScore,ISNULL(s08.SleeveLengthScore,0) SleeveLengthScore,
...
SELECT
    ...
    a.ThicknessScore,a.Waist_typeScore
INTO #tmpFinishScore
FROM #tmpFinishScore0 a
LEFT JOIN Paper_Perform_newBaseScore b(nolock) ON a.newbanid=b.banid
WHERE 1=1

SELECT
    c.papermanager,c.PMPerson,c.newbanid as finishBanid,c.paperRedoCount,d.generalScore,
    CASE
        WHEN (ISNULL(c.paperRedoCount,0)>=0) and (ISNULL(c.paperRedoCount,0)<1) THEN d.generalScore*0.7 -- 一次通过
    ...
    INTO #tmpQatScore
    FROM #tmpPaperQat c
    LEFT JOIN #tmpFinishScore d on c.papermanager=d.papermanager AND c.PMPerson=d.PMPerson AND c.newbanid=d.newbanid
WHERE 1=1


SELECT
    DISTINCT a.paperMgr,a.paperEngineer,a.sc_number,a.styleno,
    CASE WHEN isnull(a.banid,'')<>'' THEN a.banid ELSE b.banid END as banid,
    b.PPType
INTO #tmpPaperOtherQty
FROM PaperStylenoState a(nolock)
...
SELECT
    a.paperMgr as papermanager,a.paperEngineer as PMPerson,a.banid,a.styleno,a.PPType,
    ISNULL(s01.BanxingScore,0)+ISNULL(s02.CollartypeScore,0)+ISNULL(s03.Design_styleScore,0)+
    ISNULL(s04.DresslengthScore,0)+ISNULL(s05.FabcharacterScore,0)+ISNULL(s06.FabnameScore,0)+
    ISNULL(s07.SexTypeScore,0)+ISNULL(s08.SleeveLengthScore,0)+ISNULL(s09.SleevetypeScore,0)+
    ISNULL(s10.Styletype2Score,0)+ISNULL(s11.ThicknessScore,0)+ISNULL(s12.Waist_typeScore,0) AS generalScore,
...
FROM #tmpPaperOtherQty a(nolock)
INNER JOIN dsg_bi.dbo.Ban_Lastver_MBT07 b(nolock) on Substring(a.banid,1,12)=Substring(b.matioid,1,12)
INNER JOIN ban_makebill_head c(nolock) on b.mb_number=c.mb_number
LEFT JOIN Paper_Banxing s01(nolock) on c.Stylenametype=s01.Stylenametype AND c.banxing=s01.banxing
LEFT JOIN Paper_Collartype s02(nolock) on c.Stylenametype=s02.Stylenametype AND c.Collartype=s02.Collartype
LEFT JOIN Paper_Design_style s03(nolock) on c.Stylenametype=s03.Stylenametype AND c.Design_style=s03.Design_style
...
SELECT
    a.papermanager,a.PMPerson,a.banid,a.styleno,a.generalScore,
...
SELECT
    papermanager,PMPerson,Banid,ISNULL(paperRedoCount,0) as paperRedoCount,
...
SELECT
    '未分配部门' as PaperDept,papermanager,PMPerson,receiveBanid as Banid,paperRedoCount,generalScore,
...
FROM(
    SELECT
        papermanager,PMPerson,receiveBanid,generalScore,
...
GROUP BY
    papermanager,PMPerson,receiveBanid,paperRedoCount,generalScore ,
    BanxingScore,CollartypeScore,Design_styleScore,DresslengthScore,
    FabcharacterScore,FabnameScore,SexTypeScore,SleeveLengthScore,
    SleevetypeScore,Styletype2Score,ThicknessScore,Waist_typeScore
