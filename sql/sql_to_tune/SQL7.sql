...
inner join dsg_prelist_detail b(nolock) on a.pre_number=b.pre_number
left join b_item d(nolock) on b.item_code=d.item_code
left join b_itemcate e(nolock) on d.treeID=e.treeid
WHERE a.confirmflag=1
    and a.bossflag=1
    and b.reqjcomfirmflag=1
    AND isnull(b.cancelflag,0)=0
    and isnull(b.completeflag,0)=0
    AND (((a.reqdept='ZHDGB' or b.dsgdeptname='ZHDGB')
            AND a.publicitemcheckflag=1
            and a.fadcheckflag=1)
            or (a.reqdept<>'ZHDGB' and b.dsgdeptname<>'ZHDGB'))
    AND isnull(b.reqtcomfirmflag,0)=0
    AND b.reqpconfirmflag=1
    AND isnull(b.digestionstoreflag,0)=0
    AND isnull(b.digestionstockflag,0)=0
    AND isnull(b.YuliustoreFlag,0)=0
 --  AND (isnull(b.gdcancleflag,0)=0  or  isnull(b.gdcanclecheckflag,0)=0 )
    and a.season like '2017冬季'
    and b.mattype='辅料'
    and e.seriestype like '织带类'
...
LEFT JOIN (
    ...
    group by b.pre_number,b.pre_line)c1 on (a.pre_number=c1.pre_number and a.pre_line=c1.pre_line)
left join(
    select distinct a.season,b.item_code
    from dsg_prelist_head a(nolock)
    inner join dsg_prelist_detail b(nolock) on a.pre_number=b.pre_number
    where (b.isorderluseflag=1 or b.qtyotherorder>0)  and isnull(b.cancelflag,0)=0) f on a.season=f.season and a.item_code=f.item_code
LEFT JOIN(
    select *
    from b_item_color(nolock)
    where synflag=1) g on g.item_code=a.item_code and g.color_name=a.color_name and g.color_desc=a.color_desc

WHERE (a.needqty)-(isnull(c1.yfpqty,0))-(isnull(a.cancleQty,0))-(isnull(a.cancleQty2,0))>0


select *
from #tmp order by pre_number,pre_line
