SELECT distinct a.banid,a.bantypedesc,a.season,a.sex,a.makedeptname,aa.factorydesc,           
  ...
  INTO #tmpbanprocess         
  FROM dsg_bi.dbo.Ban_Process_DateQry a(nolock) inner join           
        b_styleno p(nolock) on a.banid=p.banid      
            
    left join     
    (Select distinct a.factorydesc,b.banid from DCForward_head(nolock) a  Left join     
     DCForward_detail(nolock) b on b.dcf_number=a.dcf_number )aa on aa.banid=a.banid  where 1=1 
   and p.season  =  '2017冬季'
