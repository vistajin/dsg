select
    matioid,sc_number,max(isnull(mb_number,'')) as mb_number,max(isnull(mbversion,'')) as mbversion
into #tmplastban
...
SELECT
    b.mb_number,b.mb_line,b.item_code,b.des3 as parts
INTO #tmpbanitem
FROM dsg.dbo.ban_makebill_itemlist b(nolock)
INNER JOIN dsg.dbo.ban_makebill_head a (nolock) on a.mb_number = b.mb_number
WHERE 1=1
...
select
    distinct a.season,a.salesSeason,a.mb_number,a.matioid,a.styleno,a.sc_number,a.mbversion,a.makedeptdesc,a.PPType,a.dsgdeptconfirmdate
into #tmpfirstmbt07
...
SELECT
    item0.item_code,item0.item_desc,itemcate.seriestype
INTO #tmpitem
FROM dsg.dbo.b_item item0(nolock)
Inner join dsg.dbo.b_itemcate itemcate(nolock) on item0.treeID = itemcate.treeID
WHERE usestate='启用'


SELECT c.season,c.salesSeason,c.banid,c.styleno,c.sc_number,c.styledesc,c.makedeptname,c.makedeptdesc,c.trackgroupnm,c.tracknm
INTO #tmpstyleno
...
WHERE 1=1
    ...
    and c.banid not in (
        ...
        select banid from dsg.dbo.od_head (nolock) where ISNULL(stylecancel,'')<>'' or ISNULL(IsStopFlag,0)=1
        union  --过滤掉订货资料里非采用的款，快单除外（款号以7开头）
        select banid from dsg.dbo.od_head (nolock) where ISNULL(IsActiveFlag,0)=0 and styleno not like '7%'
    )


select
    distinct b.mb_number,b.mb_line,
...
into #tmpmatsource
from dsg.dbo.ban_makebill_itemcolor b(nolock)
INNER JOIN dsg_bi.dbo.Ban_Lastver_MBT07 a (nolock) on a.mb_number = b.mb_number
WHERE 1=1
    and (salesSeason>'2017' or salesSeason='2016冬季')


select
    matioid,sc_number,istransferseason,MAX(mb_number) mb_number,MAX(mbversion) mbversion
into #tmpts
from dsg.dbo.ban_makebill_head (nolock)
where (salesSeason>'2017' or salesSeason='2016冬季')
...
into #tmpfgb
from dsg.dbo.item_fenggebanapply_detail fd(nolock)
Inner join dsg.dbo.item_fenggebanapply_head fh(nolock) on fd.fgb_number=fh.fgb_number
where confirmflag=1
    and (fd.season>'2017' or fd.season='2016冬季' or fd.season='2016秋季')


select
    distinct a.DocNo AS resdocno,b.styleno,b.MatNo
into #tmpres
from  dsg.dbo.MaterialSend_head a(nolock)
INNER JOIN dsg.dbo.MaterialSend_DetailD b(nolock) ON a.DocNo=b.DocNo
...
select
    banid,MAX(scandatetime) as bantryfactdate
into #tmptd
FROM(
    select
        banid,scandatetime
...
into #tmpins
from dsg.dbo.materialcollect_detail a(nolock)
inner join dsg.dbo.materialcollect_head b(nolock) on a.DocNo=b.DocNo
...
INTO #tmpdb
FROM #tmplastban a2
Inner join #tmpbanitem b on a2.mb_number = b.mb_number
Inner join #tmpfirstmbt07 a0 ON a0.matioid=a2.matioid and a0.sc_number=a2.sc_number
Inner join #tmpitem item on b.item_code = item.item_code --头板+齐色板单的最早版本的设计经理审核日期
LEFT JOIN #tmpstyleno c on a2.matioid=c.banid and a2.sc_number = c.sc_number
LEFT JOIN #tmpmatsource ms on ms.mb_number=b.mb_number and ms.mb_line=b.mb_line
LEFT JOIN #tmpts ts on ts.matioid=a2.matioid and ts.sc_number=a2.sc_number
LEFT JOIN #tmpfgb fgb ON b.item_code=fgb.item_code and (c.season=fgb.season or fgb.season='通用季')
LEFT JOIN #tmpres res on res.styleno=a2.matioid and res.MatNo=b.item_code
LEFT JOIN #tmptd td on a0.matioid=td.banid
LEFT JOIN #tmpys ys on a0.matioid=ys.banid               --原板(落单)试衣OK时间
LEFT JOIN #tmpzs zs on a0.sc_number = zs.sc_number
LEFT JOIN #tmpins ins ON (c.salesSeason=ins.season or ins.season='通用季') and (b.item_code= ins.MatNo)
