...
-- =============================================
CREATE PROCEDURE [dbo].[getBanBatchAndCrockID] AS
BEGIN
	DECLARE @days int
	SET @days=180
	...
	from materialPasterColor_detaild(nolock) a
	LEFT join(
		SELECT b.banid,b.item_code,b.color_name,d.req_number
		FROM req_detail(NOLOCK) a
		LEFT JOIN req_detaild(NOLOCK) b ON a.req_serial=b.req_serial AND a.req_line=b.req_line
		inner JOIN (SELECT req_number,req_serial FROM req_head(NOLOCK))d ON a.req_serial=d.req_serial)b ON a.OrderNo=b.req_number AND a.MatNo=b.item_code AND a.colorid=b.color_name
