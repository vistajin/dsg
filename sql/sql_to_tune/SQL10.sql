...
Select
	...
	a.avnoresweight Qtyi,a.Wunit as Qunit,a.storelength stocklength,--库存数量
	a.storeweight stockweight,a.Wunit,a.conversion,a.avnoreslength as Length,a.Lunit,a.avnoreslength as usablelength,--实际可用
	...
	case when pre.unit<>a.Lunit and pre.unit='磅' then (pre.needqty-ISNULL(pre.cancleQty,0)-ISNULL(pre.cancleQty2,0))*a.sampleqty/a.conversion else (pre.needqty-ISNULL(pre.cancleQty,0)-ISNULL(pre.cancleQty2,0)) end preneedqty,
	(isnull(c.outlength,0)+ISNULL(d.sendlength,0)) bcklength,
	case when a.storeno in ('B','C') and a.avnoreslength-((case when pre.unit<>a.Lunit and pre.unit='磅' then pre.rtnqty*a.sampleqty/a.conversion else pre.rtnqty end)-isnull(c.outlength,0)-ISNULL(d.sendlength,0))>=0 then ((case when pre.unit<>a.Lunit and pre.unit='磅' then pre.rtnqty*a.sampleqty/a.conversion else pre.rtnqty end)-isnull(c.outlength,0)-ISNULL(d.sendlength,0)) when a.storeno in ('B','C') and a.avnoreslength-((case when pre.unit<>a.Lunit and pre.unit='磅' then pre.rtnqty*a.sampleqty/a.conversion else pre.rtnqty end)-isnull(c.outlength,0)-ISNULL(d.sendlength,0))<0 then a.avnoreslength when a.storeno in ('A') and a.avnoreslength-(pre.rtnqty-isnull(c.outlength,0)-ISNULL(d.sendlength,0))>=0 then  pre.rtnqty-isnull(c.outlength,0)-ISNULL(d.sendlength,0) when a.storeno in ('A') and a.avnoreslength-(pre.rtnqty-isnull(c.outlength,0)-ISNULL(d.sendlength,0))<0 then a.avnoreslength else a.avnoreslength end as prerestqty
...
left join (
	Select c.pre_number,c.pre_line,SUM(a.length) as outlength from MaterialOut_Detail a
	...
	where c.resno is null and b.SendNo not like 'BCK%' and c.pre_number is not null
		and a.MatNo like '%16FW-NJR0155%'
	group by c.pre_number,c.pre_line) c on c.pre_number=reqd.pre_number and c.pre_line=reqd.pre_line    --已出库
left join (
...
where 1=1 and a.storelength>0 and a.storeweight>0 and a.avnoreslength >0 and a.avnoresweight>0
	...
	AND a.LocationID <>'BA666666'
	and a.MatNo  like '%16FW-NJR0155%'
	and a.Season  like '%2018春季%'
	and a.colordesc like '%绿色%'
