  ...
  SELECT DISTINCT                               
    ...
    b.batch,b.bantracknm,b.rptgroupnm,b.reqjcomfirmdatetime ,                               
                                       
    c.needqty,c.fpqty,c.rtnqty,                             
    ...
    c.cancelconfirmflag2,c.cancelconfirmuser2,c.cancelconfirmdate2,                             
                                
d.req_serial, d.req_line,d.poprice,d.dahuoprice,d.jjprice,d.minoyearprice,d.minoyearcacprice,d.needqty as sumneedqty,                               
    d.nowsupply_code ,d.nowsupply_desc,d.supply_code,d.supply_desc,d.supply_colorname,d.supply_colordesc ,d.completeflag,                              
    d.indatedetail,d.dporevdate,d.reqdate ,       
  ...
  FROM dsg_prelist_head a(nolock) inner join                                                                                                                                 
       dsg_prelist_detail b on (a.pre_number=b.pre_number) left join                                
                                         
       req_detaild c(nolock) on b.pre_number=c.pre_number and b.pre_line=c.pre_line left join                                         
       req_detail d(nolock) on c.req_serial=d.req_serial and c.req_line=d.req_line   left join                
       req_head e (nolock) on c.req_serial=e.req_serial  left join                
                   
       b_item g(nolock) on b.item_code=g.item_code left join                                                                                           
       b_itemcate h(nolock) on g.treeID=h.treeID left join                                
                       
      (select distinct banid,styleno,makedeptdesc from b_styleno                                
      )p on b.banid=p.banid  left join                       
      (select b.OrderSerial as req_serial,b.Orderline as req_line,min(c.checkdate) as rkdate                             
        ...
        group by b.OrderSerial,b.Orderline ) j                                 
           on d.req_serial=j.req_serial and d.req_line=j.req_line                                                                   
  WHERE 1=1                                 
  
 and a.season = '2017冬季'
 and isnull(g.itemtype,'')='01'
 and e.reqdept in (select reqdept from reqrange where userid='SA') and e.podept in (select podept from porange where userid='SA')                                                     
    select a1.*,      
            f.poindate,f.poconfirmdate, f.podate,                               
     ...
     left join     /* 采购单|开单日期、确认日期(F)、供应商复期 */                                                  
          (select b.req_serial,b.req_line,                              
                  min(a.indate) as poindate,min(confirmdate) as poconfirmdate ,min(podate) as podate                                                                                 
             from poreq_head a(nolock )                                 
              inner join poreq_detail b(nolock )  on a.preq_number=b.preq_number                                                      
             group by b.req_serial,b.req_line  ) f                                  
          on a1.req_serial=f.req_serial and a1.req_line=f.req_line                                 
     left join      /* 回货单|回货确认日期(G)*/                                                 
           (select b.req_serial,b.req_line,min(a.confirmdate) as revconfirmdate                                                                                  
              ...
              group by b.req_serial,b.req_line ) i                                  
          on a1.req_serial=i.req_serial and a1.req_line=i.req_line                                  
     left join      /* 收货单|收货日期(I)、板房收货确认 */                                   
           (select b.req_serial,b.req_line,min(a.rtndate) as rtndate ,min(confirmsenddate) as confirmsenddate                                                                                 
              ...
              group by b.req_serial,b.req_line ) k                                  
           on a1.req_serial=k.req_serial and a1.req_line=k.req_line                                 
     left join      /* 预收单|预收日期(H)、入库单|入库日期(J)*/                                                 
           (select b.OrderSerial as req_serial,b.Orderline as req_line,min(b.checkdate) as ysindate ,                                                    
              ...
              group by b.OrderSerial,b.Orderline ) j                                 
           on a1.req_serial=j.req_serial and a1.req_line=j.req_line                                 
     left join      /* 数量|领用数量(按申请) */                                                 
           (select SUM(Length) as prellqty,pre_number,pre_line                                                                               
              from  MaterialSend_DetailD b(nolock )                                                    
              group by b.pre_number,b.pre_line ) l                                 
           on a1.pre_number=l.pre_number and a1.pre_line=l.pre_line                                 
     left join      /* 数量|领用数量(按板单) */                                                 
           (select SUM(Length) as banllqty,matno,colorid,colordesc,styleno                                                                               
             from  MaterialSend_DetailD b(nolock )                                                    
             group by matno,colorid,colordesc,styleno ) z                     
             on a1.item_code=z.matno and a1.color_name=z.colorid                                 
             and a1.color_desc=z.colordesc                                 
             and ( a1.banid=z.styleno)                                  
     ...
     (/*select orderserial,orderline,unqualified,result,MAX(a.checkdate) as checkdate                              
      ...
      from dsg_checkout_detail b(nolock)                               
      Inner join dsg_checkout_head a(nolock) on a.det_number=b.det_number                             
      group by orderserial,orderline    
      ) c on c.orderserial=b.orderserial and c.orderline=b.orderline    
      where a.checkdate=c.checkdate                                     
     )o                               
     on o.orderserial=a1.req_serial and o.orderline=a1.req_line                                 
