...
select
    ...
    d.Weight inweight,
       ...
       case WHEN isnull(f.length,0)-isnull(h.length,0) < 0 THEN b.length-isnull(e.length,0)
            ...
            else b.length-isnull(e.length,0)-isnull(i.length,0)-(isnull(f.length,0)-isnull(h.length,0))end  as  avnoreslength,  --实际可用数量= 库存-正常未出-预留未出-预留剩余
...
left join (select batchid, SUM(length)as length, SUM(qtyi) as qtyi
		   ...
		   group by BatchID) f on a.batchid=f.batchid  --f 预留数量
left join (select batchid, SUM(length)as length, SUM(qtyi) as qtyi
           from materialsend_detaild a(nolock)
		   inner join materialsend_head  d(nolock) on a.docno=d.docno
           where d.checkflag=1 and a.resno like 'RES%'
           group by BatchID) h on a.batchid=h.batchid  --h  领预留数量
left join (select batchid, SUM(length)as length, SUM(qtyi) as qtyi
		   from materialsend_detaild a(nolock)
		   inner join materialsend_head d(nolock) on a.docno=d.docno
		   left join materialout_head c(nolock) on a.docno=c.sendno
