...
Select 
     ...
     case when a.storeno in ('B','C') then 'Ö÷ÁÏ' when a.storeno='A' then '¸¨ÁÏ' else '' end as itemtype1,
     ...
     case when pre.unit<>a.Lunit and pre.unit='°õ' then (pre.needqty-ISNULL(pre.cancleQty,0)-ISNULL(pre.cancleQty2,0))*a.sampleqty/a.conversion
     ...
     case when a.storeno in ('B','C') and (a.avnoreslength-((case when pre.unit<>a.Lunit and pre.unit='°õ' then pre.rtnqty*a.sampleqty/a.conversion else pre.rtnqty end)-isnull(c.outlength,0)-ISNULL(d.sendlength,0))>=0) then ((case when pre.unit<>a.Lunit and pre.unit='°õ' then pre.rtnqty*a.sampleqty/a.conversion else pre.rtnqty end)-isnull(c.outlength,0)-ISNULL(d.sendlength,0))
 
          when a.storeno in ('B','C') and (a.avnoreslength-((case when pre.unit<>a.Lunit and pre.unit='°õ' then pre.rtnqty*a.sampleqty/a.conversion else pre.rtnqty end)-isnull(c.outlength,0)-ISNULL(d.sendlength,0))<0) then a.avnoreslength
 ...
 left join (
     ...
     group by a.pre_number,a.pre_line) d on d.pre_number=reqd.pre_number and d.pre_line=reqd.pre_line  --Î´³ö¿â
 
  
 where 1=1 and a.storelength>0 and a.storeweight>0 and a.avnoreslength>0 and a.avnoresweight>0
 
       and (k.lockflag is null or k.lockflag<>'1') and a.MatNo like '%18EK-PWS0172%' --and a.colorid like '%01-003%';
 
select * from #tmp a where --a.MatNo like '%18EK-PWS0172%';
 
         a.colorid like '%01-003%';
