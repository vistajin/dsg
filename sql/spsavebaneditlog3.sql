USE [dsg]
GO
/****** Object:  StoredProcedure [dbo].[spsavebaneditlog3]    Script Date: 14/09/2017 11:27:34 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE procedure [dbo].[spsavebaneditlog3]
...
begin
    ...
    begin try
        if @trancount = 0
            ...
            save transaction spsavebaneditlog3;
...
lbexit:
    ...
    begin catch
        ...
        if @xstate = 1 and @trancount = 0
            rollback
        if @xstate = 1 and @trancount > 0
            rollback transaction spsavebaneditlog3;

        raiserror ('spsavebaneditlog3: %d: %s', 16, 1, @error, @message) ;
        return;
    end catch   
end